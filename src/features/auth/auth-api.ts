...
export const getAuth = async () => {
	...
	return {
		new_loc: loginForm?.querySelector<HTMLInputElement>(
			"input[name='new_loc']",
		)?.value,
		nonce: loginForm?.querySelector<HTMLInputElement>(
			`input[name='${NONCE_VALUE}']`,
		)?.value,
		cookies: response.rawHeaders["set-cookie"].map(
...
export const signIn = async (rgm: string, password: string) => {
	...
	const response = await fetchWithoutRedirect<string>(
		getApi("/webapps/login/"),
		{
			method: "POST",
			body: Body.form({
				user_id: rgm,
	...
	const bbSession = response.rawHeaders["set-cookie"]
