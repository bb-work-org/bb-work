...
import { type CourseResult } from "@/@types/courses";
...
function Root({ courseRoot }: { courseRoot: CourseResult }) {
  ...
  const staticBannerQuery = useGetCourseStaticBannerQuery(`nature${(courseRoot.courseCardColorIndex % 20) + 1}_thumb`);

  const { isLoading, data: bannerUrl } = courseRoot.course.banner ? bannerQuery : staticBannerQuery;
  ...
  return isLoading ? (
    ...
    <Body
      ...
      image={
        bannerUrl ? (
          <Image src={bannerUrl} alt={courseRoot.course.name} className="object-cover" fill />
        ) : (
          <ImageSkeleton />
        )
      }
